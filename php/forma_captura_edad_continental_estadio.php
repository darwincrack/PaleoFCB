...
*********************************************************************-->
	...
	// extrae valores del formulario
		...
		if($id_glacial_interglacial_FK=='NULL' & $glacial_interglacial=='NULL')
		{
			...
			{
			  $id_glacial_interglacial_FK= $data->llave;
			}
	...
	****************-->
	<?php
		...
		if ($id_glacial_interglacial_FK=="NULL" & $glacial_interglacial!="NULL")
		{
			...
			while ($data = pg_fetch_object($qu)) 
			{
			  $id_glacial_interglacial_FK= $data->id_glacial_interglacial_fk;
			}		
			...
			// INSERTA LOS VALORES
			$query = "INSERT INTO t_glacialinterglacial 
	...
	****************-->
	<?php
		...
		$query = "SELECT 
					...
					t_estadios.estadios as estadios
				FROM 
					t_estadios
				WHERE 
					id_estadios IN 
					(SELECT DISTINCT 
						glaciacionescompleta.estadio
					FROM 
					...
					AND 
						glaciacionescompleta.periodoGlacial = $id_glacial_interglacial_FK);";
		...
		while ($data = pg_fetch_object($qu)) 
		{
		  ...
		  if($data->estadios!='No Disponible')
				 {
				echo "<option value = '".$data->id_estadio_fk."'>".$data->estadios.''."</option>"; 
				 }
